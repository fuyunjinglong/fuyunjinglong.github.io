...
面试答：**1 句概念 + 1 句触发 + 3 个典型应用**

> BFC 是块级格式化上下文，可以理解成页面中一个独立的布局容器，内部元素不会影响外部，外部也不会影响内部。
> 触发条件常见有：根元素、浮动元素、绝对定位元素、overflow 不为 visible、display: inline-block / flow-root / flex / grid 等。
> 它主要能解决三类问题：
> 1）清除浮动，解决父容器高度塌陷；
> 2）防止垂直方向的 margin 折叠；
> 3）实现自适应两栏布局，避免浮动元素覆盖普通内容。
...
> - **DOM 结构变化**：添加、删除可见 DOM 节点。
> - **几何属性变化**：修改 `width`、`height`、`padding`、`margin`、`border-width`、`font-size` 等。
> - **位置属性变化**：修改 `position`、`top`、`left`、`float`、`clear` 等。
> - **获取特定属性值**（**极其易踩坑**）：当读取 `offsetWidth`、`offsetHeight`、`clientTop`、`scrollTop`、`getComputedStyle()` 等属性时，**为了返回最新的准确值，浏览器会被迫立即执行一次同步回流！**
> - **全局触发**：页面初次渲染、浏览器窗口大小改变（`resize`）。

3.如何减少回流：

> - 需要要对元素进行复杂的操作时，可以先隐藏（display:none”），操作完成后再显示
> - 需要创建多个DOM节点时，使用DocumentFragment创建完后一次性的加入document
> - 缓存Layout属性值，如：varleft=elem.offsetLeft这样，多次使用left只产生一次回流
...
**Sass 和 SCSS 是同一个 CSS 预处理器的两种语法格式，不是两种语言**。

> - sass文件：严格的缩进表示层级，CSS代码粘贴进来会报错
> - .scss文件：带有{}表示层级，100% 兼容CSS文件。**SCSS 是 CSS 的超集**

## CSS常用3种引入方式

> - 行内样式：极少使用
> - 内部样式：将 CSS 代码写在 HTML 文档的 `<head>` 标签内的 `<style>` 标签中，复用性差
> - 外部样式：独立的 `.css` 文件中，通过 `<link>` 标签（或 `@import`）引入，复用性好，需要额外网络请求
...
- 行内样式（1000）
- ID选择器（0100）
- 类class选择器 / 属性选择器 / 伪类选择器（0010）
- 标签div选择器 / 伪元素选择器（0001）
- 关系选择器 / 通配符选择器（0000）
...
>
> 它的核心作用是让元素在父容器中自动撑满剩余空间。其中 `flex-basis: 0%` 非常关键，它告诉浏览器在分配空间时忽略元素自身内容的尺寸，从而实现真正的严格等分。
>
> 它最常用的场景是处理‘固定与自适应’的布局，比如左侧固定宽度侧边栏，右侧 `flex: 1` 撑满主内容区；或者移动端底部导航栏，每个 Tab 都设为 `flex: 1` 实现完全等分；还有 Header/Footer 固定、中间内容区自适应滚动的布局。
>
> 另外需要注意它和 `flex: auto` 的区别，`flex: auto` 会基于内容尺寸分配剩余空间，而 `flex: 1` 是无视内容绝对等分。”
...
| :----------- | :----------------------------- | :-------------------------------------- |
| **归属**     | HTML 标签                      | CSS 语法规则                            |
| **加载方式** | 并行加载（与HTML解析同步）     | 串行加载（需等包含它的CSS下载完才请求） |
| **DOM操作**  | 可通过 JS 动态创建、修改、删除 | 无法通过 JS 操作                        |
| **兼容性**   | 所有浏览器                     | IE5+ 支持（现代浏览器均支持）           |
| **书写位置** | HTML 的 `<head>` 中            | CSS 文件开头或 `<style>` 标签内         |
| **性能影响** | 优（无额外请求延迟）           | 差（容易导致“闪屏”或加载延迟）          |
...
> “伪类和伪元素的核心区别在于：**伪类是选择已有元素的特殊状态，伪元素是创建源代码中不存在的虚拟节点。**
>
> 从语法上区分，伪类用单冒号，如 `:hover`、`:nth-child`；伪元素用双冒号，如 `::before`、`::selection`。
>
> 从原理上讲，伪类相当于动态添加 class，不影响 DOM 结构；而伪元素相当于在 DOM 内部插入了一个真实的 span 标签，它可以拥有自己的盒模型属性（如宽高、边距），并且必须配合 `content` 属性使用。比如清除浮动利用 `::after`，就是因为在父容器末尾生成了一个虚拟的块级元素来执行 `clear: both`。”
...
> 1. **布局革命**：引入了 Flexbox 和 Grid 布局，让我们彻底告别了浮动和表格布局的痛点，让各种居中和复杂排版变得简单。
> 2. **动画能力**：通过 `transition` 和 `animation` 配合 `transform`，我们可以实现流畅的硬件加速动画，减少了对 JavaScript 动画库的依赖。
> 3. **视觉增强**：`border-radius`、`box-shadow`、`filter` 等属性，让我们不再需要为了圆角和阴影去切图，提高了开发效率。
> 4. **响应式设计**：`@media` 媒体查询和 `vw/vh` 视口单位，使得一套代码适配多端屏幕成为可能。
> 5. 此外，还有丰富的**选择器**和 **RGBA/HSLA** 颜色模型等。”

## canvas与svg的区别？

一句话：**Canvas 是“像素画板”（位图），SVG 是“图形文档”（矢量图）。**

面试答题模板：

> “Canvas 和 SVG 的核心区别在于渲染机制：Canvas 是基于像素的位图，属于即时模式，画完就忘；SVG 是基于 XML 的矢量图，属于保留模式，浏览器维护着图形的 DOM 树。
>
> 这导致了它们在三个方面的显著差异：
>
> 1. **事件与交互**：SVG 可以直接给内部图形绑定 DOM 事件，而 Canvas 内部没有 DOM 节点，只能靠坐标碰撞检测来模拟事件。
> 2. **性能**：Canvas 性能受画布尺寸和重绘频率影响，适合游戏、大量粒子等高频绘制；SVG 性能受节点数量影响，节点过多会卡顿，适合图标、常规图表等少量高精图形。
> 3. **缩放**：SVG 天然支持无限放大不失真，而 Canvas 放大后会模糊，需要开发者手动处理高分屏适配。
>
> 因此，如果是做游戏、像素处理或超大数据量渲染选 Canvas；如果是做图标、地图、强交互且节点不多的图表选 SVG。
