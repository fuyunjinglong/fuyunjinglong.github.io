...
> - **MCP(Model Context Protocol)**:标准化的"工具连接协议"，解决 Agent 如何安全接入外部系统(API、数据库、浏览器)的问题；
> - **Function Calling**:模型层的原生能力，让模型能输出结构化的工具调用请求；
> - **Skills**:更高层的"知识封装"，内容可以是纯指令(无需脚本)，也可以捆绑脚本、文档、模板，跨 Agent 可移植。
