...
一句话：函数式编程主要体现为：**函数是一等公民**、**纯函数**、**不可变数据**以及**声明式编程**风格。

**1. 核心特性（关键得分点）**

> - **函数是一等公民：**
>   这是JS函数式编程的基础。意味着函数可以像普通值一样被传递、赋值给变量、作为参数传给其他函数，或者作为返回值返回。这使得**高阶函数**成为可能。
>
> - **纯函数：**
>   这是函数式编程的灵魂。纯函数满足两个条件：
>
>   1. **相同的输入总是返回相同的输出。**
>   2. **无副作用**，即不修改外部状态，不修改参数，不进行I/O操作。
>
>   - *意义：* 纯函数使得代码易于测试、易于缓存（Memoization），且线程安全。
>
> - **不可变性：**
>   数据一旦创建就不能被修改。如果需要改变，必须基于原数据创建一个新的副本。
>
>   - *实践：* 在React开发中，我们严禁直接 `push` 或 `splice` 修改state，而是使用扩展运算符 `...` 或 `immer` 库来返回新对象。
>
> - **声明式编程 vs 命令式编程：**
>   函数式编程推崇声明式。
>
>   - *命令式：* 告诉计算机“怎么做”，充斥着 `for` 循环、`if` 判断和临时变量。
>   - *声明式：* 告诉计算机“做什么”，代码更接近自然语言，逻辑更清晰。

**2. 常用技术与工具（展示实战能力）**

在实际开发中，我们经常使用以下函数式编程技术：

> - **高阶函数：**
>   接受函数作为参数或返回函数的函数。最典型的就是 `Array.prototype` 上的方法：`map`、`filter`、`reduce`。
>   - *举例：* 将数组 `[1, 2, 3]` 每一项乘以2，命令式写法需要写 `for` 循环，而 `arr.map(x => x * 2)` 就是声明式的函数式写法。
> - **柯里化：**
>   将一个多参数函数转化为一系列单参数函数的技术。
>   - *作用：* 实现参数复用（偏函数）和延迟执行。
> - **组合与管道：**
>   将多个单一功能的函数组合成一个复杂的函数，数据像水流过管道一样经过层层处理。例如 `f(g(h(x)))`，可以使用 `compose` 函数来实现，保证数据流动的清晰性。

**🚀 总结与对比（升华与辩证思考）**

> **1. 函数式编程 vs 面向对象编程 (OOP)：**
>
> - **OOP：** 封装数据和行为，强调对象的状态和继承。适合处理复杂的业务逻辑和状态管理。
> - **FP：** 强调数据的流动和变换，避免状态共享。适合处理数据转换、并发计算和UI组件渲染。
>
> **2. 优缺点权衡：**
>
> - **优点：** 代码简洁、可预测性强、易于测试、易于并发（无共享状态）。
> - **缺点：** 学习曲线陡峭；过度使用可能导致代码晦涩难懂；由于强调不可变性，频繁创建新对象可能带来**性能开销**（虽然现代JS引擎优化得很好，但在极高性能场景下仍需权衡）。

**总结：**
在现代前端开发中，我们通常不会极端地追求纯粹的函数式编程，而是采用**“函数式为主，面向对象为辅”**的混合范式。例如在React组件中，用函数式思想处理数据和渲染，用面向对象思想处理复杂的业务模型。

**💡 面试官可能的追问（预判）**

> 1. 能否手写一个柯里化函数？
>    - *答题思路：* 利用闭包收集参数，判断参数个数是否达到原函数参数长度，决定是返回新函数还是执行原函数。
> 2. 如何理解Redux中的函数式编程思想？
>    - *答题思路：* Redux的Reducer必须是纯函数（State + Action = New State）；Middleware利用了高阶函数和组合的概念。
> 3. 你提到了副作用，在项目中如何处理副作用？
